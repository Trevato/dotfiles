...
> k3s is not currently active on this Mac. Colima runs docker-only. To re-enable:
> `colima kubernetes start` (adds k3s) / `colima kubernetes stop` (removes it).
> Use `kubectl config use-context colima` for the local cluster.
